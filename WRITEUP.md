...
**Supabase over rolling my own auth.** Magic-link auth in <20 LOC, and RLS-ready if I ever wanted to expose the DB directly to the client. The tradeoff was fighting the transaction-mode pooler (no prepared statements, which meant `prepare: false` in the Postgres client _and_ a hand-rolled migration runner because `drizzle-kit migrate` hangs against the pooler). Worth it overall — auth is a problem I didn't want to build twice.
